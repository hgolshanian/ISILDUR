...
sendCommand(bytearray([0, 0, 0, 0, 0, 15, 0, 43, 13, 1, 0, 0, 96, 154, 0, 0, 0, 0, 2, 80, 100]))
...
def gantry_callback(data):
 ...
 if data.f == 3:
  ...
  #data.y=6000
  yr=data.y
  data.y=data.y*87.5   #85.7 is for tranfering to motor rotation 
  inty = int(data.y)
  hex_y = hex(inty)


  hex_y = hex_y[2:].zfill(8)  # Removing '0x' and padding zeros to make it 8 characters


  byte_1 = int(hex_y[6:8], 16)
  byte_2 = int(hex_y[4:6], 16)
  byte_3 = int(hex_y[2:4], 16)
  byte_4 = int(hex_y[0:2], 16)
