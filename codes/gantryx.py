...
rospy.init_node('gantry_move',anonymous=True)
...
def gantry_callback(data):
 ...
 if data.f == 2: 
  ...
  # print(data.x)
  xr=data.x
  data.x=(-1*data.x)+50   #to make the calibration the same as ur3
  data.x=data.x*87.5   #85.7 is for tranfering to motor rotation
  intx = int(data.x)
  hex_x = hex(intx)


  hex_x = hex_x[2:].zfill(8)  # Removing '0x' and padding zeros to make it 8 characters


  byte_1 = int(hex_x[6:8], 16)
  byte_2 = int(hex_x[4:6], 16)
  byte_3 = int(hex_x[2:4], 16)
  byte_4 = int(hex_x[0:2], 16)
